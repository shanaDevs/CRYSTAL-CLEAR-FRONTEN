...
export default function EditProductForm() {
  ...
  const [images, setImages] = useState(locationData.state.images);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (images.length == 0) {
        results = locationData.state.images;
      }
      ...
      await axios.put(import.meta.env.VITE_API_URL + "/product/"+productId, product, {
        headers: {
    ...
    <div className="w-full h-full rounded-lg flex justify-center items-center bg-gray-700">
      <div className="w-[500px] h-[600px] flex flex-col rounded-lg shadow-lg items-center">
        ...
        <div className="w-[380px] h-[100px] flex justify-between items-center rounded-lg">
          <Link
            to={["/admin/products"]}
            className="text-white rounded-lg p-[10px] w-[180px] text-center bg-red-500 hover:bg-red-700"
          >
            Cancel
          </Link>
          <button
            onClick={handleSubmit}
            className=" text-white rounded-lg p-[10px] w-[180px] text-center bg-green-500 hover:bg-green-700"
          >
            Update Product
          </button>
        </div>
